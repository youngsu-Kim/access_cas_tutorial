...
In the past decade or two, there has been a significant development in computing resources, and several NSF awards focused on expanding computing resources for academic research and education. 
...
The main goal of this tutorial is to present a streamlined procedure and demonstrate how to effectively use and monitor resources for SageMath and Macaulay2. 

> Change it to general and add SageMath and Macaulay2####

It will be particularly useful for those who do not have on-campus computing resources as the resources in this tutorial (ACCESS) are available for academic purposes at no cost. For questions, comments, or suggestions, please contact Youngsu Kim at youngsu.kim@csusb.edu. 

## Acknowledgments

Rewrite this.
This project is supported in part by the NSF grant (award number 2230127) lead by Dr. Mary Thomas at San Diego Supercomputing Center. The author participated in this project as a 2023 cyberinfrastructure professional trainee from California State University San Bernardino.
...
This tutorial focuses on ACCESS and assumes that one has allocations on ACCESS. To request allocations on ACCESS (and it is free), please go visit this [page](https://allocations.access-ci.org/prepare-requests-overview#comparison-table) for details. There is no fee. Once you have allocations, it is time to choose the supercomputing center(s) for your project.
...
**Warning:** One will be charged by the resources we request not by the portion of resources one utilizes. For instance, let's say one asks for 4 CPUs, but ends up using only 2. Then one is still charged for the SUs for 4 CPUs. It is important to monitor resource usage and we discuss it in this tutorial, in Section #### Add link to the sectionxxxx.
...
Indiana Jetstream2 is unique as it provides full desktop-like environments. Several supercomputing centers offer only a portal and/or SSH access. Since it is most close to a PC, we cover this first. 
